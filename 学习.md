...
- **自动控制**: 根据土壤湿度自动启停水泵
- **报警提示**: 环境参数超出安全范围时触发蜂鸣器,并通过 LoRa 上报
...
✅ **任务间协作**: 传感器数据要传递给显示任务,环境状态要传递给 LoRa 发送任务
...
| LoRa通信 | 低(可以缓冲) | 1秒 | ✅ 是 | 周期上报,与采集节奏一致 |
...
void StartSensorTask(void *argument) {
    ...
    for(;;) {
        ...
        if (farmState.temperature > farmSafeRange.maxTemperature) {
            CheckRangeFloat("temperature_high", farmState.temperature);
        }
...
**职责:**
1. 读取最新农场状态
2. 通过 UART3 发送 JSON 到 E32 模块
3. 周期到达后再次上报

**为什么这样设计?**

- **周期上报**: 每秒发送一次最新状态,与采集节拍一致
- **优先级 Low**: 低优先级,不影响实时性要求高的任务
- **直接发送**: 通过 UART3 直接发送 JSON 字符串
- **数据来源**: LoRaTask 读取共享全局变量 `farmState`

**新手容易犯的错误:**
❌ 用轮询方式检查队列,浪费 CPU
❌ 发送完消息后忘记释放内存,导致缓冲区越界
...
```
InputTask (High) > SensorTask (Normal) > ScreenTask (BelowNormal5) ≈ LoRaTask (Low)
```
...
| 任务 | 触发条件 | 典型代码 |
|------|---------|----------|
| LoRaTask | 到达发送周期 | `osDelay(1000);` |

**为什么这样设计?**

- **LoRaTask**: 周期上报最合理,用固定周期发送即可
...
❌ **问题 2: 无法保证实时性**
  - 如果 `sendLoRaAlert()` 耗时较长(比如发送 UART),会阻塞当前任务
  - SensorTask 被阻塞,传感器采集延迟,报警不及时

❌ **问题 3: 无法解耦**
  - SensorTask 直接调用 LoRaTask 的函数,耦合严重
  - 如果 LoRaTask 的实现改变,SensorTask 也要修改
...
#### **场景 1: SensorTask → LoRaTask(状态传递)**

**问题:** SensorTask 采集到最新状态,需要让 LoRaTask 周期上报给网关

**解决方案:** 使用共享全局状态 `farmState`
...
✅ **全局变量更合适:**
  - 只关心最新值,不需要历史缓存
  - LoRaTask 按固定周期发送,逻辑更直接
  - SensorTask 与 LoRaTask 的职责更清晰
...
4. 调用 `CheckRangeFloat("temperature_high", 35.5)`

#### **步骤 3: SensorTask 发送最新状态上报**

```c
// Core/App/lora_e32.c
FarmState snapshot = farmState;
snprintf(payload, sizeof(payload),
         "{\"temp\":%.1f,\"hum\":%.1f,\"soil\":%u,\"light\":%u,\"rain\":%u,\"pump\":%u,\"alarm\":%u}",
         snapshot.temperature, snapshot.humidity, snapshot.soilMoisture,
         snapshot.lightIntensity, snapshot.rainGauge, snapshot.waterPumpState, alarm);
LoRa_Send(payload);
```

**发生了什么?**
1. LoRaTask 读取最新 `farmState` 快照
2. 格式化 JSON 字符串
3. 通过 UART3 发送到 E32 模块
4. 网关收到后继续转发 MQTT/Web/Android
...
2. 调度器切换到其他就绪任务(比如 ScreenTask 或 LoRaTask)
3. 1 秒后,SensorTask 被唤醒,继续下一次采集

#### **步骤 6: LoRaTask 通过 UART3 发送数据**
...
**发生了什么?**
1. 通过 UART3 发送 JSON 字符串到 LoRa 模块
2. E32 透传给 ESP32 网关
3. 网关再上传到 MQTT/Web/Android
...
✅ **任务独立执行:**
  ...
  - LoRaTask 异步发送报警,不影响 SensorTask 和 ScreenTask
  - InputTask 快速响应按键,不会被 OLED 刷新阻塞

✅ **调度器自动管理:**
  - 优先级高的任务(InputTask)优先执行
  - 阻塞的任务自动让出 CPU,其他任务运行
  - 事件驱动的任务(LoRaTask)只在有事件时执行,节省 CPU
...
**问题:** LoRaTask 占用 CPU 期间,其他任务无法执行,系统卡顿
...
void SensorTask() {
    while(1) {
        char *msg = char payload[192];  // 固定缓冲区
...
| 阻塞函数占用 CPU | UART 使用 DMA 发送,并在等待完成时通过 `osDelay(1)` 让出 CPU | lora_e32.c:58 |
| 缓冲区越界 | 明确约定:SensorTask 分配,LoRaTask 释放 | SensorTask.c:43, lora_e32.c:69 |
...
| Core/App/Tasks/lora_e32.c | LoRa通信任务 | `StartLoRaTask()` |
