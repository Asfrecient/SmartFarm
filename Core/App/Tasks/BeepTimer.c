...
void BeepTimerCallback(void *argument){
    ...
    if(beep_state == 0){    
        // 状态0：开启蜂鸣器
        HAL_TIM_PWM_Start(&htim4, TIM_CHANNEL_4);  // 启动PWM输出
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_4, 15);  // 设置占空比（音量）
        //beep_state = 1;  // 切换到开启状态
    }else{
        // 状态1：关闭蜂鸣器
        HAL_TIM_PWM_Stop(&htim4, TIM_CHANNEL_4);  // 停止PWM输出
        __HAL_TIM_SET_COMPARE(&htim4, TIM_CHANNEL_4, 0);  // 设置占空比为0
